...
export class ScrapReadDaoSupabaseService {
  ...
  getProductsScrapById(scrapId: number): Observable<ProductScrapEntity[]> {
    ...
    const promise = this.supabaseService.getSupabaseClient().schema('public').from('v_product_scrap_report')
      .select('*')
  ...
  getAll(): Observable<ScrapEntity[]> {
    return from(
      this.supabaseService.fromScrap('scrap')
        .select('*')
  ...
  getAllSummary(): Observable<ScrapSummaryEntry[]> {
    return from(
      this.supabaseService.getSupabaseClient().schema('public').from('v_scrap_summary')
        .select('*')
        .order('created_at', { ascending: false })
    ).pipe(
  ...
  getProductsWithSources(productId?: number): Observable<ProductWithSourcesEntity[]> {
    ...
    let query = this.supabaseService.getSupabaseClient().schema('public').from('v_product_sources_aggregated')
      .select('*');
