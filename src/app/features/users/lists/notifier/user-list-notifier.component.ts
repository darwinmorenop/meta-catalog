...
@Component({
    selector: 'app-user-list-notifier',
    standalone: true,
    imports: [
        CommonModule,
        MatCardModule,
    ...
    templateUrl: './user-list-notifier.component.html',
    styleUrl: './user-list-notifier.component.scss'
})
export class UserListNotifierComponent {
    ...
    listResource = rxResource({
        stream: () => this.listService.getNotifier(this.filteredUserIds())
    });
    ...
    onViewList(item: any) {
        ...
        this.router.navigate(['/users/lists', item.id], {
            queryParams: showName ? { showName: 'true', redirectTo: 'stock-notifier' } : { redirectTo: 'stock-notifier' }
        });
    }
