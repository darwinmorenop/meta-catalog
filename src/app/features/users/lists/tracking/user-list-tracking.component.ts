...
@Component({
    selector: 'app-user-list-tracking',
    standalone: true,
    imports: [
        CommonModule,
        MatCardModule,
    ...
    templateUrl: './user-list-tracking.component.html',
    styleUrl: './user-list-tracking.component.scss'
})
export class UserListTrackingComponent {
    ...
    listResource = rxResource({
        stream: () => this.listService.getTracking(this.filteredUserIds())
    });
    ...
    onEditList(item: ListEntity) {
        const showName = this.scope() !== 'Personal';
        this.router.navigate(['/tracking', item.id, 'edit'], {
            queryParams: showName ? { showName: 'true' } : {}
        });
    }
}
